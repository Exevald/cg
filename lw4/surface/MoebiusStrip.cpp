...
MoebiusStrip::Vertex MoebiusStrip::CalculateVertex(const float u, const float v) const
{
	...
	const float intensity = (vert.position.z + m_width) / (2.0f * m_width);
	vert.color = glm::vec4(
		0.5f + 0.5f * cosf(u),
		intensity,
		0.5f + 0.5f * sinf(u),
...
{
	m_mesh.clear();
	for (int i = 0; i <= m_uSegmentsCount; ++i)
	{
		const auto u = (2.0f * M_PI * i) / m_uSegmentsCount;
		std::vector<Vertex> triangleStrip;
		for (int j = 0; j <= m_vSegmentsCount; ++j)
		{
			const auto v = -m_width
				+ (2.0f * m_width * static_cast<float>(j)) / static_cast<float>(m_vSegmentsCount);
			triangleStrip.push_back(CalculateVertex(static_cast<float>(u), v));
		}
		m_mesh.push_back(triangleStrip);
	}
}
